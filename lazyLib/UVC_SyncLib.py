...
class UVC_API_Sync(object):
    ...
    def login(self):
        ...
        if r.status_code is not 200:
            self.logger.critical(
                f"Failed to log into the DVR. Check the error {r.json()}"
            )
            sys.exit(1)
    ...
    def logout(self):
        ...
        if r.status_code is not 200:
            self.logger.critical(
                f"Failed to log out of the DVR. Check the error {r.text}"
            )
            sys.exit(1)
    ...
    def camera_info(self):
        ...
        r = self.session.get(
            f"{self.url}/api/2.0/bootstrap",
            cookies=self.session.cookies.update({"cameras.isManagedFilterOn": "false"}),
        )
        if r.status_code is not 200:
            self.logger.critical(
                f"Failed to obtain the camera data. Check the error {r.text}"
            )
            sys.exit(1)
        elif r.status_code is 200:
        ...
        try:
            ...
            self.logger.error(
                "The UVC didn't provide the bootstrap data for the cameras."
            )
            ...
            if len(bootstrap_data["data"][0]["cameras"]) > 0:
                ...
                self.logger.critical(
                    "The bootstrap endpoint didn't provide the correct data. Exiting."
                )
                sys.exit(1)
    ...
    def clip_meta_data(self, clip_id_list):
        ...
        with click.progressbar(
            clip_id_list,
            length=clip_id_list_len,
            label="Clip Data Downloaded",
            show_eta=False,
            show_percent=False,
            show_pos=True,
        ) as bar:
            for id in bar:
                ...
                req = requests.Request(
                    "GET", f"{self.url}/api/2.0/recording/{id}", cookies=meta_cookies
                )
                prepped = req.prepare()
        ...
        for c in camera_meta_data_list:
            ...
            if c["inProgress"]:
                self.logger.warning(
                    f"Skipping clip ID {c['_id']}, it's still recording"
                )
            else:
                ...
                human_start_time = strftime(
                    "%d_%m_%Y-%H:%M:%S", gmtime(startTime / 1000.0)
                )
                fullFileName = f"{human_start_time}-{mod_cam_name}.mp4"

                self.dict_info_clip.update(
                    {
                        clip_id: clip_info(
                            clip_id,
                            startTime,
        ...
        for count, clip in enumerate(self.dict_info_clip, 1):
            ...
            with open(file_path, "wb") as f:
                with click.progressbar(
                    ...
                    length=num_chunks,
                    label=f"Downloading Video {self.dict_info_clip[clip].fullFileName}",
                    show_percent=True,
                    show_eta=False,
                ) as bar:
                    for data in bar:
                        f.write(data)
            self.logger.info(
                f"Done downloading file {self.dict_info_clip[clip].clip_id}"
            )
    ...
    def outputPathCheck(self, output_path):
        ...
        if not output_path.is_dir() and output_path.exists():
            self.logger.critical(
                f"Output path {output_path} is not a directory but it exists, specify a directory."
            )
            sys.exit(1)
