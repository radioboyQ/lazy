...
class EmpirePushover(object):
    ...
    def password_auth(self):
        ...
        req = Request(
            "POST",
            "https://{}:{}/api/admin/login".format(self.hostname, self.port),
            json=uname_password,
        )
            ...
            try:
                ...
                if not self.no_progress_bar:
                    ...
                    with click.progressbar(
                        length=self.time_delay,
                        label="Time Until Next Check",
                        show_eta=False,
                    ) as bar:
                        while tick > 0:
                            tick = tick - 1
    ...
    def check_new_agents(self, response_json):
        ...
        for a in response_json["agents"]:
            if a["ID"] not in self.known_sessions:
                try:
                    msg = "[+] New agent found.\n[+] ID: {}\n[+] Hostname: {}\n[+] Internal IP: {}\n[+] Username: {}".format(
                        a["ID"], a["hostname"], a["internal_ip"], a["username"]
                    )
                    logging.info(msg)

                    if not self.dry_run:
                        pushover_response = self.p.notify(
                            user=self.pushover_user_token,
                            token=self.pushover_app_token,
                            title="New Agent Phoned Home",
                            message=msg,
                        )
                        ...
                        if pushover_response.status == "Failure":
                            ...
                            # ToDo: Fix this garbage with proper error trapping
                            logging.error(
                                "[!] Something went wrong when we were trying to use Pushover!\n[*] Check your API keys and ensure they are correct.\n[!] Exiting."
                            )
                            raise click.Abort()
    ...
    def _empire_request(self, method, url, data_json=None, headers=None):
        ...
        if url.startswith("/api/"):
            full_url = "https://{}:{}{}?token={}".format(
                self.hostname, self.port, url, self.empire_api
            )
        else:
            full_url = "https://{}:{}/api/{}?token={}".format(
                self.hostname, self.port, url, self.empire_api
            )
        ...
        resp = self.session.send(
            prepped_req, proxies=self.proxies, verify=self.ssl_verify
        )
    ...
    def test(self):
        ...
        if pushover_response.status == "Failure":
            ...
            click.echo(
                "[!] Pushover encountered an issue. Check your API keys and try again."
            )
        else:
            # ToDo: Convert these statements to logging statements
