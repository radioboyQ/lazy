...
@click.option(
    "-n",
    "--refresh-noaa",
    help="Refresh the cached NOAA satellite ID numbers.",
    is_flag=True,
    default=False,
)
@click.option(
    "-p",
    "--refresh-prediction",
    help="Refresh the satellite predictions",
    is_flag=True,
    default=False,
)
@click.option(
    "-a", "--refresh-all", help="Refresh all caches", is_flag=True, default=False
)
@click.option(
    "--num-days",
    help="Set the number of days to request. Default is 5",
    type=click.INT,
    default=5,
)
@click.pass_context
def cli(ctx, refresh_noaa, refresh_prediction, refresh_all, num_days):
    ...
    n2yo = n2yolib(
        "EY8PE3-34D8LM-G7DEUT-3TNS",
        configOptions["pushover"]["token"],
        configOptions["pushover"]["user"],
    )
    ...
    prediction_cache_pth_dt_period = pendulum.now() - pendulum.from_timestamp(
        os.path.getmtime(prediction_cache_pth), tz=pendulum.now().timezone_name
    )

    # If it's older than 12 hours, refresh it
    if (
        prediction_cache_pth_dt_period.in_hours() >= 12
        or refresh_prediction
        or refresh_all
        or not prediction_cache_pth.exists()
    ):
        ...
        if not norad_cache_pth.exists():
            ...
            n2yolib.norad_cache_refresh(
                norad_cache_pth, configOptions["satellite"]["satellite-names"]
            )
        ...
        for name in sat_id_dict:
            ...
            sat_pass_data[name] = n2yo.radio_pass(
                id, observer_lat, observer_lng, observer_alt, num_days, min_elevation
            ).json()
...
@cli.command(
    "next-pass",
    help="Display when the next satellite is going to pass overhead",
    context_settings=CONTEXT_SETTINGS,
)
@click.argument("N-PASSES", default=3, type=click.INT)
@click.option(
    "--pushover-only",
    help="Only send a notification via Pushover, do not show anything on STDOUT. Useful for scripting.",
    is_flag=True,
    default=False,
)
@click.pass_context
...
@cli.command(
    "schedule-capture",
    help="Schedule an 'at' job to run and capture the SDR data.",
    context_settings=CONTEXT_SETTINGS,
)
@click.pass_context
