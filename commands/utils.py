...
@click.group(
    name="utils",
    short_help="A collection of tools which are useful but don't fit anywhere else.",
...
@cli.command(
    "share", help="Mount or unmount a shared drive.", context_settings=CONTEXT_SETTINGS
)
@click.option(
    "-s",
    "--share-name",
    help="Name of the share from the config file.",
    default="QNAP",
    type=click.STRING,
)
@click.option(
    "--status",
    help="Show the current status of the mounted drive.",
    is_flag=True,
    default=False,
)
@click.pass_context
def share(ctx, share_name, status):
    ...
    if share_name.lower() in configOptions["share"]:
        ...
        share_status = os.path.ismount(
            configOptions["share"][share_name.lower()]["mount_point"]
        )
        ...
        if status:
            ...
            if share_status:
                click.secho(
                    "[!] The shared drive is already mounted. Attempting to unmount it.",
                    fg="white",
                )
                returnValue = lazyTools.mount_changer(
                    configOptions["share"][share_name.lower()]["mount_point"]
                )
                if returnValue[0] == True:
                    ...
                    if returnValue[1].startswith("Unmount failed for"):
                        ...
                        click.secho(
                            "[*] Check for files that are open on the remote share. This also includes command prompts pointed to the remote file system.",
                            fg="red",
                        )
            else:
                ...
                filled_path = configOptions["share"][share_name.lower()]["path"].format(
        ...
        raise click.BadParameter(
            "The share name '{}' doesn't appear to exist. Check the config file and try again.".format(
                ctx.params["share_name"]
            )
        )
...
@cli.command(
    "wiki-backup",
    help="Backup the local Doku Wiki to an Amazon S3 bucket.",
    context_settings=CONTEXT_SETTINGS,
)
@click.pass_context
def backup(ctx):
    ...
    backup_filename = "personalWiki_{date}.zip".format(
...
@cli.command(
    "backup",
    help="Backup various files and folders I care about.",
    context_settings=CONTEXT_SETTINGS,
)
@click.option(
    "-s",
...
def backup(ctx, server):
    ...
    try:
        ...
        raise click.ClickException(
            "Invalid server configuration. Check the config file and try again."
        )

    # Connect to NYC server with tunnel
    vps = lazyTools.SSHTools(
        server_config["ssh_middle_user"], server_config["ssh_middle_host"]
    )
...
@click.option(
    "-w",
    "--write",
    help='Append string to udev file. This requires "sudo" privileges. *Not working!* ',
    is_flag=True,
    default=True,
)
@click.option(
    "-p",
    "--udev-path",
    help="Udev configuration file path. Default: /etc/udev/rules.d/60-persistent-net.rules",
    default="/etc/udev/rules.d/60-persistent-net.rules",
    type=click.Path(
        exists=True, dir_okay=False, writable=False, resolve_path=True, allow_dash=True
    ),
)
@click.pass_context
def udev_rename(ctx, mac_addr, name, write, udev_path):
    ...
    udev_str = 'SUBSYSTEM=="net", ACTION=="add", DRIVERS=="?*",ATTR{address}=="{mac_addr}",ATTR{dev_id}=="0x0", ATTR{attr_type}=="1",KERNEL=="*", NAME="{name}"'.format(
        address=address,
        mac_addr=mac_addr,
        dev_id=dev_id,
        attr_type=attr_type,
    ...
    if write:
        if os.geteuid() != 0:
            ...
            click.echo(
                f"echo '{udev_str}' | sudo tee -a /etc/udev/rules.d/60-persistent-net.rules"
            )
        elif os.geteuid() == 0:
...
@cli.command(
    "maps",
    help="Open Google Maps with a specific user ID.",
    context_settings=CONTEXT_SETTINGS,
)
