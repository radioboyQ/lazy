    ...
    if value is None:
        if param.name is "start_time":
            ...
            raise click.BadParameter(
                f"I' being called for {param.name} which is wrong. WTF MATE."
            )
    try:
...
async def run_multiple_clients(hosts: list, username: str, sh_command: str):
    ...
    for i, result in enumerate(results, 0):
        if isinstance(result, Exception):
            ...
            non_zero_exit.append(
                "Host {} exited with a non 0 error. Error code: {}\n".format(
                    hosts[i], result.exit_status
                )
            )
...
@click.group(
    "denhac",
    help="Commands to help with denhac server administration.",
...
@click.option(
    "-s", "--server", help="Remote server to connect to.", default="denhac.org"
)
@click.option(
    "-c",
    "--cmd",
    help="Command to run on the remote hosts. Defaults to restarting Odoo.",
    type=click.STRING,
    default="sudo systemctl restart odoo",
)
@click.option(
    "-u",
    "--username",
...
def restart_odoo(ctx, server, cmd, username):
    ...
    failed_task, non_zero_exit, success_task = loop.run_until_complete(
        run_multiple_clients(hosts, username, cmd)
    )
...
@click.command(
    ...
    help="Download videos for cameras for a specific timeframe.",
    context_settings=CONTEXT_SETTINGS,
)
@click.option(
    "-s",
    "--start-time",
    callback=datetime_check,
    help="Specify a start time in DD:MM:YYYY-HH:mm",
)
@click.option(
    "-e",
    "--end-time",
...
@click.option(
    "-d",
    "--hostname",
    help="Domain name, hostname or IP address for the Video controller. E.g. 127.0.0.1",
    type=click.STRING,
    required=True,
)
@click.option(
    "-p",
    "--port",
    help="Port number for the Video controller. Defaults to 7443",
    default=7443,
...
@click.option(
    "-o",
    "--output-dir",
    help="Directory to save the videos to.",
    type=click.Path(
        exists=False, file_okay=False, writable=True, resolve_path=True, allow_dash=True
    ),
)
@click.option(
    "-p", "--password", help="UVC User's password.", prompt=True, hide_input=True
)
...
def main(
    ...
    client.clip_search(
        epoch_start=start_time, epoch_end=end_time, camera_id_list=camera_id_list
    )
