...
@cli.command(
    "email-import",
    help="Import a list of emails into a GoPhish server.",
    context_settings=CONTEXT_SETTINGS,
)
@click.argument(
    "user-csv",
...
@click.option(
    "-g",
    "--group-size",
    help="Define the email group size on import.",
    default="0",
    type=click.INT,
)
@click.option(
...
@cli.command(
    "delete-groups",
    help="Delete groups that start with a given string.",
    context_settings=CONTEXT_SETTINGS,
)
    ...
    if section_name.lower() in config_options["gophish"]:
        ...
        if config_options["gophish"][section_name.lower()]["VPN_Required"]:
            ...
            if debug:
                ...
                if lazyTools.ConnectedToVPN(
                    lazyTools.parentSetting(ctx, "config_path")
                ):
                    # Connected to VPN
                    if debug:
                        click.secho("[*] Connected to VPN", fg="green")
        ...
        if group_found == False:
            click.secho(
                "[!] No groups were found that start with {} .".format(group_prefix),
                bold=True,
            )
        else:
            click.secho(
                "[*] {count} groups were found and they were deleted.".format(
                    count=group_counter
                ),
                bold=True,
                fg="green",
...
def temp(ctx, section_name):
    ...
    ssh_listen_interface = config_options["gophish"][section_name.lower()][
        "ssh_listen_interface"
    ]
    ...
    gophish_interface = config_options["gophish"][section_name.lower()][
        "gophish_interface"
    ]
    api_key = config_options["gophish"][section_name.lower()]["api_key"]
    authorized_keys_file = config_options["gophish"][section_name.lower()][
    ...
    if section_name.lower() in config_options["gophish"]:
        ...
        try:
            ...
            tasks = asyncio.gather(
                lazyTools.SSHTools.local_port_forward(
                    host,
                    ssh_port,
                    ssh_username,
                    ssh_listen_interface,
                ...
                Gophish(
                    ...
                    verify=config_options["gophish"][section_name.lower()][
                        "Verify_SSL"
                    ],
                ),
            )
